...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    #获取真实、和预测结果中，每帧的数据物体总数objects
    num_gt = len(gt_anno["label_type"])
    ...
    for i in range(num_gt):
        bbox = gt_anno["bbox"][i]
        gt_name = gt_anno["label_type"][i].lower()
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    total_dt_num = np.stack([len(a["label_type"]) for a in dt_annos], 0)
    ...
    for num_part in split_parts:
        ...
        if metric == 1 or metric==0 :
            loc = np.concatenate(
                [a["box_center"][:, [0, 2]] for a in gt_annos_part], 0)
            dims = np.concatenate(
                [a["box_size"][:, [0, 2]] for a in gt_annos_part], 0)
            ...
            loc = np.concatenate(
                [a["box_center"][:, [0, 2]] for a in dt_annos_part], 0)
            dims = np.concatenate(
                [a["box_size"][:, [0, 2]] for a in dt_annos_part], 0)
    ...
    for anno in dt_annos:
        if anno['name'].shape[0] != 0:
            if anno['name'][0] != -10:
                compute_aos = True
            break
    mAPbev = do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges, compute_aos)
    print(mAPbev)
    print(type(mAPbev))
    for j, curcls in enumerate(current_classes):
        # mAP threshold array: [num_minoverlap, metric, class]
