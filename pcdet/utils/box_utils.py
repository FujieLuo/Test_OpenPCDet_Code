...
def boxes_to_corners_3d(boxes3d):
    ...
    Returns:
    """
    ##check_numpy_to_torch的作用：判断传入来的参数是否是np.ndarray的类型，
    # 结果是： true，并且通过torch.from_numpy(boxes3d).float()将boxes3d 转换成tensor变量。
    # 返回两个量：boxes3d是tensor变量，  is_numpy是true
    ...
    #经过下面这一步，corners3d变成了3维的了
    corners3d = boxes3d[:, None, 3:6].repeat(1, 8, 1) * template[None, :, :]
    '''执行结果如下：>>> c.shape         输出：torch.Size([6, 8, 3])
    故corners3d：torch.Size([N, 8, 3])
    tensor([[[ 0.0550,  0.3554, -0.2986],
         [ 0.0550, -0.3554, -0.2986],
         [-0.0550, -0.3554, -0.2986],
         [-0.0550,  0.3554, -0.2986],
         [ 0.0550,  0.3554,  0.2986],
         [ 0.0550, -0.3554,  0.2986],
        ...
        [[ 0.1361,  0.0768, -0.2117],
         ...
         其中，下面的参数corners3d.view(-1, 8, 3)执行的结果也是这个
    '''
    #corners3d：torch.Size([N, 8, 3])，boxes3d：torch.Size([N, 7])
    #boxes3d[:, 6]表示取出最后一列的数字，也就是heading的数据
    #返回后的corners3d值是经过heading里的数据旋转变换后的点云的位置张量tensor
    corners3d = common_utils.rotate_points_along_z(corners3d.view(-1, 8, 3), boxes3d[:, 6]).view(-1, 8, 3)
    #boxes3d：（N,7）。boxes3d[:, None, 0:3] 是取了boxes3d的前三列的数据
    #corners3d =corners3d+boxes3d[:, None, 0:3] 
    #corners3d的形状是torch.Size([6, 8, 3])
    corners3d += boxes3d[:, None, 0:3]

    #最后，将corners3d转换为了numpy格式返回，numpy.ndarray（N,8,3）
